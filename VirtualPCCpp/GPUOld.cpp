...
void GPU::executeThreads(u16 opCode) {
	...
	case 2: {
		...
		byte yP0 = functionBuffer.memory[opCode + 3];
		byte yP1 = functionBuffer.memory[opCode + 4];
		byte r = functionBuffer.memory[opCode + 5] * 4;
		byte g = functionBuffer.memory[opCode + 6] * 4;
		byte b = functionBuffer.memory[opCode + 7] * 4;
		u16 xP = xP0 + (xP1 * 256);
		u16 yP = yP0 + (yP1 * 256);
		screen.pixels[yP * 320 + xP] = int(225 << 16) | int(225 << 8) | int(225);
		programCounter += 7;
		break; }
	}
...
void convertByte(u16 number, byte& b1, byte& b2) {
	if (number > 255) {
...
void GPU::spawnThreads(byte function, u16 arg1 = 0, u16 arg2 = 0, u16 arg3 = 0, u16 arg4 = 0, u16 arg5 = 0, u16 arg6 = 0, u16 arg7 = 0) {
	switch (function) {
	case 1: {
		...
		for (int i = 0; i < xPositions.size(); i++) {
			...
			if (xPos <= 255) {
				...
				int temp = (xPos - 255) / 256;
				functionBuffer.memory[(i * 5) + 1] = 255;
				functionBuffer.memory[(i * 5) + 2] = temp;
			}
			if (yPos <= 255) {
				functionBuffer.memory[(i * 5) + 3] = yPos;
				functionBuffer.memory[(i * 5) + 4] = 0;
			}else if (yPos > 255) {
				int temp = (yPos - 255) / 256;
				functionBuffer.memory[(i * 5) + 3] = 255;
				functionBuffer.memory[(i * 5) + 4] = temp;
			}
		}
		break; }
	case 2: {
		...
		for (u16 i = arg2; i < arg4; i++) {
			for (u16 j = arg1; j < arg3; j++) {
				xPositions.push_back(j);
				yPositions.push_back(i);	
			}
		}

		for (u16 i = 0; i < arg4 - arg2; i++) {
			for (u16 j = 0; j < arg3 - arg1; j++) {
				byte xpos1, xpos2, ypos1, ypos2;
				convertByte(xPositions[i * (arg3 - arg1) + j], xpos1, xpos2);
				convertByte(yPositions[i * (arg3 - arg1) + j], ypos1, ypos2);
				functionBuffer.memory[(i * (arg3 - arg1) + j) * 8 + 0] = 2;
				functionBuffer.memory[(i * (arg3 - arg1) + j) * 8 + 1] = xpos1;
				functionBuffer.memory[(i * (arg3 - arg1) + j) * 8 + 2] = xpos2;
				functionBuffer.memory[(i * (arg3 - arg1) + j) * 8 + 3] = ypos1;
				functionBuffer.memory[(i * (arg3 - arg1) + j) * 8 + 4] = ypos2;
				functionBuffer.memory[(i * (arg3 - arg1) + j) * 8 + 5] = arg5;
				functionBuffer.memory[(i * (arg3 - arg1) + j) * 8 + 6] = arg6;
				functionBuffer.memory[(i * (arg3 - arg1) + j) * 8 + 7] = arg7;
				programSize++;
			}
		}
...
void GPU::execute() {
	if (functionBuffer.Empty() == true) {
		...
		case 1: {
			...
			break; }
		case 2: {
			u16 arg1 = argBuffer.memory[commandCounter * 11 + 0] + (argBuffer.memory[commandCounter * 11 + 1] * 256);
			u16 arg2 = argBuffer.memory[commandCounter * 11 + 2] + (argBuffer.memory[commandCounter * 11 + 3] * 256);
			u16 arg3 = argBuffer.memory[commandCounter * 11 + 4] + (argBuffer.memory[commandCounter * 11 + 5] * 256);
			u16 arg4 = argBuffer.memory[commandCounter * 11 + 6] + (argBuffer.memory[commandCounter * 11 + 7] * 256);
			u16 arg5 = argBuffer.memory[commandCounter * 11 + 8];
			u16 arg6 = argBuffer.memory[commandCounter * 11 + 9];
			u16 arg7 = argBuffer.memory[commandCounter * 11 + 10];
			spawnThreads(commandBuffer.memory[commandCounter], arg1, arg2, arg3, arg4, arg5, arg6, arg7);
			commandCounter++;
			break; }
		}
	}	
